...
def _build_operator_summary(*, business_date: str, generated_at: datetime, persist: dict, publish: dict | None) -> str:
    lines = [
        f"FSJ MAIN late publish｜{business_date}｜late",
...
def main() -> None:
    ...
    parser.add_argument("--report-run-id-prefix", default="fsj-main-late")
    ...
    try:
        persisted = LateMainFSJProducer().produce_and_persist(business_date=args.business_date)
    ...
    if not blocked:
        ...
        cmd = [
            ...
            "--report-run-id", f"{args.report_run_id_prefix}:{args.business_date}:late:{stamp}",
        ]
    ...
    summary_path = root / "main_late_publish_summary.json"
