...
from ifa_data_platform.fsj import MidMainFSJProducer
...
_FLOW = MainPublishFlowConfig(
    slot="mid",
    producer_label="MidMainFSJProducer",
    summary_name="main_mid_publish_summary.json",
    artifact_type="fsj_main_mid_publish_summary",
    report_run_id_prefix_default="fsj-main-mid",
    flow_name="fsj_main_mid_publish",
)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Persist + publish A-share FSJ mid MAIN report through one canonical operator command.")
    ...
    payload = run_main_publish_flow(
        ...
        producer_factory=MidMainFSJProducer,
