"""Highfreq compatibility wrapper.
...
This module remains for direct/manual highfreq testing, operator utilities,
and compatibility only. Do not treat `highfreq.daemon --loop` as the primary
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Highfreq compatibility wrapper")
    ...
    if args.status or args.health or args.watchdog or args.retention_run:
        print(json.dumps(daemon.status(), ensure_ascii=False, indent=2, default=str))
        return

    if args.loop:
        raise SystemExit("highfreq --loop has been demoted. Use: python3 -m ifa_data_platform.runtime.unified_daemon --loop")
