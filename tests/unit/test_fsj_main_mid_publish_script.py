...
SCRIPT_PATH = Path(__file__).resolve().parents[2] / "scripts" / "fsj_main_mid_publish.py"
spec = importlib.util.spec_from_file_location("fsj_main_mid_publish", SCRIPT_PATH)
...
def test_main_persists_then_publishes_and_writes_summary(
    ...
    monkeypatch.setattr(
        ...
        lambda self: module.argparse.Namespace(
            ...
            generated_at="2026-04-23T10:20:43+00:00",
            report_run_id_prefix="fsj-main-mid",
            include_empty=False,
        ),
    )
    monkeypatch.setattr(module, "MidMainFSJProducer", lambda: _StubProducer())
    monkeypatch.setattr(
        module,
        ...
        lambda **_: {
            "workflow_handoff": {
                ...
                "state": {
                    ...
                    "selection_reason": "best_ready_candidate strongest_slot=mid qa_score=95",
            ...
            "operator_review_surface": {
                ...
                "llm_role_policy": {
                    ...
                    "boundary_modes": ["intraday_working"],
                    ...
                    "slot_boundary_modes": {"mid": "intraday_working"},
                },
            },
        },
    )
    ...
    def _fake_run(cmd: list[str], capture_output: bool, text: bool):
        ...
        assert cmd[2:] == [
            ...
            "--generated-at", "2026-04-23T10:20:43+00:00",
            "--report-run-id", "fsj-main-mid:2026-04-23:mid:20260423T102043Z",
        ]
        return module.subprocess.CompletedProcess(
            ...
            stdout=json.dumps({
                ...
                "delivery_manifest": {"package_state": "ready", "lineage": {"bundle_id": "bundle-mid-1"}},
    ...
    assert payload["persist"]["bundle_id"] == "bundle-mid-1"
    assert payload["publish"]["status"] == "ready"
    assert len(calls) == 1
    summary = json.loads((tmp_path / "main_mid_publish_summary.json").read_text(encoding="utf-8"))
    ...
    assert "FSJ MAIN mid publish｜2026-04-23｜mid" in operator_summary
    ...
    assert "selection_reason=best_ready_candidate strongest_slot=mid qa_score=95" in operator_summary
    ...
    assert "llm_boundary_modes=intraday_working llm_override_precedence=deterministic_input_contract>validated_llm_text_fields_only" in operator_summary
    assert "llm_slot_boundary_modes=mid:intraday_working llm_forbidden_decision_count=1" in operator_summary
...
def test_main_exits_nonzero_and_skips_publish_when_persist_fails(
    ...
    monkeypatch.setattr(module, "MidMainFSJProducer", lambda: _StubProducer(error=RuntimeError("db unavailable")))
