...
class _BoardStore(_DummyStore):
    ...
    def report_operator_review_surface_from_surface(self, surface: dict) -> dict:
        ...
        return {
            ...
            "operator_go_no_go": {"decision": "GO", "rationale": "quality gate and artifact integrity both pass" if artifact_id != "commodities-artifact" else "manual review is required before sending", "approver_kind": "human_operator" if artifact_id == "commodities-artifact" else "system", "approver_id": "op-commodities-1" if artifact_id == "commodities-artifact" else None, "approver_label": "commodities-reviewer" if artifact_id == "commodities-artifact" else "fsj_policy_projection", "decided_at": "2099-04-22T10:02:00Z" if artifact_id == "commodities-artifact" else None},
            ...
            "promotion_authority": {
                ...
                "approver_kind": "human_operator" if artifact_id == "commodities-artifact" else "system",
                "approver_id": "op-commodities-1" if artifact_id == "commodities-artifact" else None,
                "approver_label": "commodities-reviewer" if artifact_id == "commodities-artifact" else "fsj_policy_projection",
                "decided_at": "2099-04-22T10:02:00Z" if artifact_id == "commodities-artifact" else None,
                "approver_summary": "kind=human_operator | id=op-commodities-1 | label=commodities-reviewer | decided_at=2099-04-22T10:02:00Z" if artifact_id == "commodities-artifact" else "kind=system | id=- | label=fsj_policy_projection | decided_at=-",
...
def test_print_text_emits_operator_board_summary(capsys) -> None:
    ...
    assert "main_promotion_authority_approver_kind=system" in output
    assert "main_promotion_authority_approver_label=fsj_policy_projection" in output
    assert "main_promotion_authority_approver_summary=kind=system | id=- | label=fsj_policy_projection | decided_at=-" in output
    ...
    assert "support_commodities_promotion_authority_approver_kind=human_operator" in output
    assert "support_commodities_promotion_authority_approver_id=op-commodities-1" in output
    assert "support_commodities_promotion_authority_approver_label=commodities-reviewer" in output
    assert "support_commodities_promotion_authority_decided_at=2099-04-22T10:02:00Z" in output
    assert "support_commodities_promotion_authority_approver_summary=kind=human_operator | id=op-commodities-1 | label=commodities-reviewer | decided_at=2099-04-22T10:02:00Z" in output
...
def test_store_build_operator_board_surface_projects_review_held_db_candidate_summary(monkeypatch) -> None:
    ...
    assert payload["db_candidate_history_summary"][0]["canonical_lifecycle_reason"] == "dispatch_receipt_without_sendable_workflow"
