...
@pytest.mark.parametrize("case", LATE_MAIN_GOLDEN_CASES, ids=[case.name for case in LATE_MAIN_GOLDEN_CASES])
def test_late_main_golden_case_family_pins_third_p3_3_benchmark_slice(case) -> None:
    descriptor = describe_slot_golden_case(case)

    assert descriptor["family"] == "late_main"
    assert descriptor["slot"] == "late"
    assert descriptor["section_key"] == "post_close_main"
    ...
    assert descriptor["minimum_counts"]["object_cnt"] >= 5
